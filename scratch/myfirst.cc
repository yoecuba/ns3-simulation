...
main(int argc, char *argv[]) {
    ...
    OnOffHelper onOffAP("ns3::UdpSocketFactory", interfaces.GetAddress(1));
    onOffAP.SetAttribute("PacketSize", UintegerValue(1024));
    onOffAP.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));
    // the packet arrivalrate on AP side is 100 pkt/sec, so we set mean value as 1/100
    onOffAP.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));
    // the constanttime for sending one packet
    onOffAP.SetAttribute("DataRate", StringValue("54Mbps"));

    ApplicationContainer clientApps;
    clientApps.Add(onOffAP.Install(nodes.Get(0)));
